...
export class AddPayaPaymentProcessor1698858063514
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    const upSql = FileHelper.read(SqlScriptPaths.ADD_PAYA_PAYMENT_PROCESSOR_UP);
    await queryRunner.query(upSql);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    const downSql = FileHelper.read(
      SqlScriptPaths.ADD_PAYA_PAYMENT_PROCESSOR_DOWN,
    );
